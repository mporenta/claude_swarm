...
1. **Break Down Complex Tasks**: Split large projects into specialized subtasks for each agent
2. **Sequential Delegation**: Complete one agent's work before moving to the next when there are dependencies
3. **Parallel Work**: Multiple agents can work simultaneously on independent components
4. **Clear Instructions**: Give each agent specific, actionable requirements
5. **Review Step**: Always include a code-reviewer step at the end
...
1. **@flask-developer**: Create `app.py` with Flask setup, routing, and random value generation
2. **@flask-developer**: Create `requirements.txt` with Flask and dependencies
3. **@frontend-developer**: Create `templates/index.html` with dynamic styling that uses Flask template variables
4. **@code-reviewer**: Review all files, run flake8, check code quality
5. **Summary**: Report files created, tests passed, and next steps for user
...
1. **Start with planning**: Think about which agent should handle each component
...
### Pattern 1: Backend + Frontend + Review
```
...
@frontend-developer → Work on static assets (independent)
# Both can work simultaneously, then integrate
```
...
- **Solution**: Provide more specific requirements and context in delegation prompt
