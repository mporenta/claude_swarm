...
print("[yellow]Please use: python main.py --config yaml_files/airflow_agent_options.yaml[/yellow]")
...
class ConversationSession:
    ...
    async def start(self):
        ...
        try:
            ...
            print(
                "  1. 🆕 Start a new Apache Airflow DAG project"
            )
    ...
    async def new_dag_flow_start(self):
        ...
        business_objective = input("Business objective (what problem does this solve?): ").strip()
        data_source = input("Data source(s) (e.g., 'Enterprise SaaS API', 'dbt', '3rd Party REST API'): ").strip()
        data_destination = input("Data destination (e.g., 'S3 → Snowflake external tables', 'S3 → raw tables', SaaS application): ").strip()

        display_message("\n[bold]Scheduling & Environment:[/bold]")
        schedule_cadence = input("Schedule cadence (e.g., 'daily 1am', 'hourly', '@daily', 'None' for manual): ").strip()
        environment = input("Target environment (local/staging/prod) [default: local]: ").strip() or "local"

        display_message("\n[bold]Requirements & Constraints:[/bold]")
        downstream_consumers = input("Downstream consumers (e.g., 'DBT models', 'BI dashboards', 'None') [optional]: ").strip() or "None specified"
        special_requirements = input("Special requirements (e.g., 'rate limiting', 'batching', 'large datasets') [optional]: ").strip() or "Standard patterns"
        sla_requirements = input("SLA requirements (e.g., 'Complete within 2 hours', 'None') [optional]: ").strip() or "None specified"
...
"""
    ...
    async def airflow_app_orchestrator(self, request: str):
        ...
        """
        try:
            main_prompt = load_markdown_for_prompt("prompts/airflow_prompts/airflow-orchestrator-v2.md")
            ...
            # Use the already-connected client directly (no context manager needed)
                ...
                async for message in self.client.receive_response():
                    ...
                    if isinstance(message, ResultMessage):
                        ...
                        display_message(f"[bold cyan]🚀 To run your airflow app:[/bold cyan]")
...
if __name__ == "__main__":
    try:
        ...
        # Load markdown prompt files \
        airflow_dev_prompt = load_markdown_for_prompt("prompts/airflow_prompts/dag-developer.md")
        migration_specialist = load_markdown_for_prompt("prompts/airflow_prompts/migration-specialist.md")
        ...
        options = ClaudeAgentOptions(
            ...
            add_dirs=[str(dev_airflow_dir)],  # Grant access to dev airflow-s directory
            env={
                # dev configuration
                "dev_HOME": str(project_root / "dev"),
                "dev__CORE__airflow-S_FOLDER": str(dev_airflow_dir),
                "airflow_ENV": "development",
                "airflow_PROJECT_PATH": str(output_dir),
                # Python configuration
                "PYTHONPATH": "/home/dev/.pyenv/versions/3.13.5/lib/python3.13/site-packages",
            },
            agents={
                "airflow-developer": AgentDefinition(
                    description="Expert airflow DAGdeveloper.",
                    prompt=airflow_dev_prompt,
                    tools=["Read", "Write", "Edit", "Bash"],
                    model="haiku",
                ),
                "frontend-developer": AgentDefinition(
                    description="Expert airflow migration specialist.",
                    prompt=migration_specialist,
                    tools=["Read", "Write", "Edit"],
                    model="haiku",
                ),
                "code-reviewer": AgentDefinition(
                    description="Expert Python Code review and best practices specialist.",
