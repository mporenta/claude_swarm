...
def _build_agent_definitions(agent_config: Mapping[str, Any]) -> dict[str, AgentDefinition]:
    ...
    for agent_name, config in agent_config.items():
        ...
        prompt_path = config_dict.pop("prompt_path", None)
        prompt = config_dict.pop("prompt", None)

        if prompt_path:
            prompt = load_markdown_for_prompt(prompt_path)

        if prompt is None:
            raise ValueError(
                f"Agent '{agent_name}' must define either 'prompt_path' or 'prompt'."
            )
