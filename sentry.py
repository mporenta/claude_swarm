...
async def searchcwd_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(
            f"Error searching directories for user {user_id}: {e}", exc_info=True
        )
        await update.message.reply_text(
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(
        f"Received message from user {user_id} ({user.username}): {user_message[:50]}..."
    )

    # Create Sentry transaction for this message handling
    with sentry_sdk.start_transaction(
        op="telegram.message", name="handle_message"
    ) as transaction:
        # Set user context
        sentry_sdk.set_user(
            {"id": str(user_id), "username": user.username or "unknown"}
        )
        ...
        with sentry_sdk.start_span(
            ...
            agent_span.set_data(
                "gen_ai.prompt", user_message[:200]
            ...
            try:
                ...
                if session_data:
                    ...
                    if session_id:
                        logger.info(
                            f"Resuming session for user {user_id}: {session_id}"
                        )
                            ...
                            if isinstance(message, AssistantMessage):
                                for block in message.content:
                                    if isinstance(block, TextBlock):
                                        ...
                                        logger.info(
                                            f"Creating Sentry span for tool: {block.name}"
                                        )
                                        with sentry_sdk.start_span(
                                        ...
                                        logger.info(
                                            f"Sentry tool span created for: {block.name}"
                                        )
                                ...
                                if (
                                    ...
                                    llm_span.set_data(
                                        "gen_ai.cost_usd", message.total_cost_usd
                                    )

                                if (
                                    hasattr(message, "duration_ms")
                                    and message.duration_ms
                                ):
                                    llm_span.set_data(
                                        "gen_ai.duration_ms", message.duration_ms
                                    )

                                if (
                                    hasattr(message, "duration_api_ms")
                                    and message.duration_api_ms
                                ):
                                    llm_span.set_data(
                                        "gen_ai.api_duration_ms",
                                        message.duration_api_ms,
                                    )

                                if hasattr(message, "num_turns") and message.num_turns:
                                    llm_span.set_data(
                                        "gen_ai.num_turns", message.num_turns
                                    )
                ...
                if tool_uses:
                    ...
                    await context.bot.send_message(
                        chat_id=update.message.chat_id, text=tool_summary
                    )
                ...
                agent_span.set_data(
                    "tools_used", [t.replace("🔧 ", "") for t in tool_uses]
                )
